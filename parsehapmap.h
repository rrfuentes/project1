...
#define CHUNKSIZE1 5 //500 //10
#define CHUNKSIZE2_1 5 //500 //10
#define CHUNKSIZE2_2 2 //2 //3
#define FIXCOL 11
...
int loadSampleNames(hid_t file,string linestream, string inipath){
    ...
    vector<string> temp;
    for(x=0;x<FIXCOL;x++){ //ignore fixed table headername
    ...
    samples[0] = (char*)malloc(samplesize*SIZE10*sizeof(char));
    for(x=0;x<samplesize;x++){ 
	samples[x]=samples[0]+x*SIZE10;
...
int writeHapmap(string inputfile,string varPath, string varName, string datafile){
    ...
    //create new file
    string miscvar, snpvar,inipath,linestream;
    hid_t file,group;
    ...
    int samcount = loadSampleNames(file,linestream,inipath);
	...
	if(fp!=NULL){
        ...
        //WRITING MISC DATA
        if(counter1==CHUNKSIZE1 || (fp==NULL && counter1>0)){ //saving of slab
